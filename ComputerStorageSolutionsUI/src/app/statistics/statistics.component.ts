...
export class StatisticsComponent implements OnInit {
  ...
  leastPopularProductChart: Chart | null = null;
  mostPopularProductChart: Chart | null = null;
  unitsSoldInPriceRangeChart: Chart | null = null;
  ...
  ngOnInit() {
    ...
    loadLeastPopularProductBtn.addEventListener('click', () => {
      ...
      if (selectedDate) {
        ...
        this.showPopup('No date selected');
      }
    });
    ...
    const loadBtn = <HTMLButtonElement>document.getElementById('loadUnitsSoldBtn') as HTMLInputElement;
    if (loadBtn) {
      loadBtn.addEventListener('click', () => this.loadUnitsSoldInPriceRange());
    }else{
      this.showPopup('No value found')
    }
  }
  ...
  onPriceChange() {
    ...
    if (this.minPrice >= this.maxPrice && this.maxPrice !== 0) {
      this.showPopup('Max Price must be greater than Min Price.');
    }
  }
  ...
  renderLeastPopularProductChart() {
    ...
    if (this.leastPopularProductChart) {
      this.leastPopularProductChart.destroy(); // Clear existing chart
    }
    if (ctx) {
      const context = ctx.getContext('2d');
      if (context) {
        ...
        this.showPopup('Failed to get 2D context for least popular product chart.');
      }
    } else {
      this.showPopup('Canvas element for least popular product chart not found.');
    }
  }  
  ...
  renderMostPopularProductChart() {
    ...
    if (this.mostPopularProductChart) {
      this.mostPopularProductChart.destroy(); // Clear existing chart
    }
    if (ctx) {
      const context = ctx.getContext('2d');
      if (context) {
        ...
        this.showPopup('Failed to get 2D context for mostt popular product chart.');
      }
    } else {
      this.showPopup('Canvas element for most popular product chart not found.');
    }
  }

  // Load units sold in a specified price range
  loadUnitsSoldInPriceRange() {
    const minPriceInput = <HTMLInputElement>document.getElementById('minPrice');
    const maxPriceInput = <HTMLInputElement>document.getElementById('maxPrice');
    ...
    if (!isNaN(minPrice) && !isNaN(maxPrice) && minPrice < maxPrice) {
      this.apiService.getUnitsSoldInPriceRange(minPrice, maxPrice).subscribe(data => {
        ...
        this.renderUnitsSoldInPriceRangeChart();
      }, error => {
        this.showPopup('Error loading data for units sold in price range.');

      });
    } else {
      this.showPopup('Please enter a valid price range.');
    }
  }

  // Method to render the chart for units sold in price range
  renderUnitsSoldInPriceRangeChart() {
    const ctx = <HTMLCanvasElement>document.getElementById('unitsSoldInPriceRangeChart');
    if (this.unitsSoldInPriceRangeChart) {
      this.unitsSoldInPriceRangeChart.destroy(); // Clear existing chart
    }
    if (ctx) {
      const context = ctx.getContext('2d');
      if (context) {
        ...
        this.unitsSoldInPriceRangeChart=
        new Chart(context, {
          type: 'bar',
          data: {
            labels: labels,
        ...
        this.showPopup('Failed to get 2D context for units sold in price range chart.');
      }
    } else {
      this.showPopup('Canvas element for units sold in price range chart not found.');
    }
  }
...
// Load orders placed by a specific customer in a specific month
  ...
  //   this.apiService.getCustomerProductsOrderedInQuarter(this.customerId, 2024, 4).subscribe(data => {
